...
describe('PaymentsService (unit)', () => {
  it('applies payment FIFO across invoices', async () => {
    ...
    const paRepo = { create: jest.fn((x) => x), save: jest.fn(async (x) => x), find: jest.fn().mockResolvedValue([]) } as any;
    const paymentRepo = { create: jest.fn((x) => x), save: jest.fn().mockImplementation(async (p) => ({ id: 10, ...p })), find: jest.fn() } as any;
